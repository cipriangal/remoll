...
double fTh_min = 0.1*deg;
double fTh_max = 5.0*deg;
double beamE = 100;//MeV

void getElastic(G4double theta,
		G4double &Q2,G4double &W2,G4double &eOut,
		G4double &xSec,G4double &fWeight, G4double &asym);
void getQuasiElastic(G4double theta,G4double Q2,G4double W2,G4double eOut,
		     G4double &xSec,G4double &fWeight, G4double &asym);
void getInelastic(G4double theta,G4double Q2,G4double W2,G4double eOut,
		  G4double &xSec,G4double &fWeight, G4double &asym);
void getKinematics(double &th, double &Q2, double &W2,double &eOut,int rn);


int main(int argc, char** argv){

  //double phaseSpaceFactor = 2.0*pi*(cos(fTh_min) - cos(fTh_max));
  return 0;
}
...
void getKinematics(double &th, double &Q2, double &W2,double &eOut, int rn) {
  ...
  if(rn){
    th = acos(CLHEP::RandFlat::shoot(cos(fTh_max), cos(fTh_min))); // radians
    eOut=0;
    Q2=0;
    W2=0;
...
void getInelastic(G4double theta,G4double Q2,G4double W2,G4double eOut,
  ...
  G4double W1 = F1/proton_mass_c2; 
  G4double xsect = MOTT*(F2/(Nu/GeV) + 2.0*T2THE*(W1*GeV))*(beamE/GeV - electron_mass_c2/GeV);
...
void getQuasiElastic(G4double theta,G4double Q2,G4double W2,G4double eOut,
  ...
  G4double w1 = F1/(proton_mass_c2/GeV);
  G4double w2 = F2/(Nu/GeV);
  
  G4double xsect = MOTT*(w2 + 2.0*T2THE*w1)*(beamE/GeV - electron_mass_c2/GeV);
